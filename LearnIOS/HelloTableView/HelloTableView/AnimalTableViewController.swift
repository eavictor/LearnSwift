    ...
    */
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as? SpecialTableViewCell {
            ...
            cell.specialImageLabel?.text = animal
            cell.specialImageView?.image = UIImage(named: animal)
            return cell
        } else {
        ...
        if segue.identifier == "showDetail" {
            if let detailViewController:DetailViewController = segue.destination as? DetailViewController {
                if let selectedIndexPath:IndexPath = self.tableView.indexPathForSelectedRow {
                    ...
                    detailViewController.infoFromViewOne = animalArray[selectedRow]
                    detailViewController.navigationItem.title = animalArray[selectedRow]
                }
            }
        }
    }
